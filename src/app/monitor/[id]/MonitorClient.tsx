...
import { Animal, AnimalData, ConservationStatus } from '@/types/animal/types';
...
export default function MonitorClient({ animal, animalData }: Props) {
      ...
      <div className="container mx-auto px-4 py-12">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center space-x-3 mb-3">
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${status.className}`}>{status.label}</span>
                <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-white/20">{animal.isMonitored ? '● Monitored' : '○ Not Monitored'}</span>
              </div>
            ...
            <div className="bg-white/10 backdrop-blur rounded-2xl p-5 min-w-[220px]">
              ...
              <div className="text-lg font-semibold" suppressHydrationWarning>{animal.lastUpdated.toLocaleString()}</div>
              <div className="text-xs uppercase tracking-wider text-white/70 mt-4 mb-2">Status</div>
              <div className="text-lg font-semibold text-success-300">{animal.isMonitored ? 'Active' : 'Inactive'}</div>
            </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={stat.label} className="bg-white dark:bg-secondary-800 rounded-2xl p-5 shadow-soft border-l-4 border-primary-500">
            ...
            <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="rounded-xl bg-secondary-50 dark:bg-secondary-900 p-4">
                  ...
                  <div className="text-lg font-semibold text-secondary-900 dark:text-white">{abundance?.aerialSurveyCounts?.toLocaleString() ?? '—'}</div>
                </div>
                <div className="rounded-xl bg-secondary-50 dark:bg-secondary-900 p-4">
                  <div className="text-secondary-500 dark:text-secondary-400 mb-1">Camera Trap Rate</div>
                  <div className="text-lg font-semibold text-secondary-900 dark:text-white">{abundance?.cameraTrapCaptureRates?.toLocaleString() ?? '—'}</div>
                </div>
                <div className="rounded-xl bg-secondary-50 dark:bg-secondary-900 p-4">
                  <div className="text-secondary-500 dark:text-secondary-400 mb-1">Range Contraction</div>
                  <div className="text-lg font-semibold text-secondary-900 dark:text-white">{conservation?.rangeContractionPercentage ? `${conservation.rangeContractionPercentage}%` : '—'}</div>
                </div>
              </div>
            </div>
          ...
          <aside className="space-y-6">
            <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft p-6">
              ...
              <dl className="space-y-3 text-sm">
                <div><dt className="text-secondary-500 dark:text-secondary-400">Category</dt><dd className="font-medium text-secondary-900 dark:text-white capitalize">{animal.category}</dd></div>
                <div><dt className="text-secondary-500 dark:text-secondary-400">Habitat</dt><dd className="font-medium text-secondary-900 dark:text-white">{animal.habitat?.join(', ') ?? '—'}</dd></div>
                <div><dt className="text-secondary-500 dark:text-secondary-400">Data Categories</dt><dd className="font-medium text-secondary-900 dark:text-white">{animal.dataCategories.join(', ')}</dd></div>
              </dl>
            </div>
            <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft p-6 space-y-3">
              ...
              <Link href={`/animal/${animal.id}`} className="block w-full text-center px-4 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-medium transition-colors">View Full Profile</Link>
              <Link href="/monitor" className="block w-full text-center px-4 py-3 rounded-xl border border-secondary-300 dark:border-secondary-600 text-secondary-700 dark:text-secondary-300 hover:bg-secondary-100 dark:hover:bg-secondary-700 font-medium transition-colors">Monitoring Center</Link>
            </div>
          </aside>
        </div>
      </div>
