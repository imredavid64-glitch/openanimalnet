    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary-50 to-primary-50 dark:from-secondary-950 dark:to-secondary-900">
      ...
      <main className="container mx-auto px-4 py-20">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <h1 className="text-5xl font-bold text-secondary-900 dark:text-white mb-4">{t('identify.title', 'AI Species Identification')}</h1>
          <p className="text-xl text-secondary-600 dark:text-secondary-400 max-w-2xl mx-auto">
            {t('identify.subtitle', 'Upload a photo and our AI will identify the species.')}
          </p>
        </motion.div>
          ...
          >
            ...
            {preview ? (
              ...
              <>
                ...
                <h3 className="text-xl font-bold text-secondary-900 dark:text-white mb-2">{t('identify.upload', 'Upload a Wildlife Photo')}</h3>
                <p className="text-secondary-500 dark:text-secondary-400">{t('identify.dragDrop', 'Drag & drop or click to select')} — JPG, PNG, WebP</p>
              </>
            )}
          </div>
        ...
        <AnimatePresence>
          {loading && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
              ...
              <p className="text-secondary-500 mt-4">{t('identify.analyzing', 'Analyzing image with AI...')}</p>
            </motion.div>
          )}

          {results.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto">
              <h2 className="text-2xl font-bold text-secondary-900 dark:text-white mb-6 text-center">{t('identify.results', 'Identification Results')}</h2>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs text-secondary-400">{t('identify.confidence', 'confidence')}</p>
                            {r.source && (
