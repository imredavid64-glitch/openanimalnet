...
import { distanceToRouteKm } from '@/lib/geo';
...
export default function ConflictPredictor() {
  ...
  const result = useMemo(() => {
    ...
    return { score, level: riskLevel(score), risks: risks.filter((r) => r.contribution > 0).sort((a, b) => b.contribution - a.contribution) };
  }, [latNum, lngNum, valid]);
  ...
  return (
    <div className="bg-white dark:bg-secondary-800 rounded-3xl p-6 shadow-lg">
      ...
      <p className="text-sm text-secondary-500 dark:text-secondary-400 mb-5">
        Pick a region or enter a coordinate to score the risk of elephant/lion/tiger encounters. The score is
        computed from distance to the species&apos; documented migration corridors — transparent, not black-box.
      </p>
      ...
      {computed && valid && result && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-2">
          ...
          <p className="text-xs text-secondary-400 dark:text-secondary-500 mt-4 border-t border-secondary-100 dark:border-secondary-700 pt-3">
            Model: risk decays exponentially with distance to documented corridors (half at ~35 km, negligible past ~150 km),
            weighted by species encounter danger and IUCN status. Educational model — not a substitute for on-the-ground assessments.
          </p>
        </motion.div>
      )}
    </div>
